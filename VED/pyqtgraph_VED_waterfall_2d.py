#=============================================
# File Name: pyqtgraph_VED_waterfall_2d.py
#
# Requirement:
...
def radarExec():
	...
	if  fn != prev_fn:
		...
		if v8len != 0:
			...
			if len(d) > 0:
				d_np = np.array(d)
				d_std = StandardScaler().fit_transform(d_np)
				sample_point = 4 # default 5
				db = DBSCAN(eps= 1.0, min_samples=sample_point).fit(d_std) # 1.2
				...
				#mask = (labels == -1)
				sensorA = []
				#sensorA = np.array(d_np[~mask])
				sensorA = np.array(d_np)
				#print("len:{:}   sensorA:\n{:}".format(len(sensorA),sensorA))
				
				'''
				wA = pointInBed(X_lo,X_hi,Y_lo,Y_hi,2,sensorA0)
				sensorA = np.array(wA)
				'''
				print("pointA({:}): pointA\n{:}".format(len(sensorA),sensorA))
			...
			xxe =  xx[0::2]
			xxo =  xx[1::2]
			xxA = np.array(xxe) + np.array(xxo)
			xs = np.array(xxA).reshape(24,32)
			np.set_printoptions(threshold=np.inf)
			print(xs)
			'''
			#print("lenXA:{:}  xxn:{:}   ".format(len(xxA),len(xxn)))
			#st = sortByLimitXY(1.0,vs.reshape(48,64)) 
			print("=============================================================")
			#print(vs) 
			v8A = vs
...
#
# total range: 3mx3m = 48 * 64 every grid xc = 3/48 = 6.25cm  yc = 3/64 = 4.68cm
#
# bed area: 1.2m(w) * 2m(h) 
#
#
